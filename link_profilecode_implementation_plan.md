...
### ✅ ステップ1: `link.profilecode.codes` の実装 - 完了

**実装状況**:
- ✅ `link.profilecode.codes` 用のリポジトリ/プロジェクトを作成
- ✅ `/join` 処理を実装（`path.startsWith("/join")` を使用）
- ✅ `apple-app-site-association` ファイルを配置（正しい `paths` 設定）
- ✅ `assetlinks.json` ファイルを配置
- ✅ ディープリンクは正常に動作
...
### ✅ ステップ2: `profilecode.codes/join` のリダイレクト実装 - 完了
...
### ✅ `link.profilecode.codes` の実装 - 完了

- [x] リポジトリ/プロジェクトを作成
- [x] `index.html` を実装（パス判定: `path.startsWith("/join")`）
- [x] カスタムスキーム: `com.profilecode.profilecode://` を使用
- [x] `.well-known/apple-app-site-association` を配置
- [x] `_config.yml` で `.well-known` をインクルード
- [x] デプロイして動作確認
...
### ✅ `profilecode.codes` の修正 - 完了

- [x] `join.html` を作成（リダイレクト処理）
- [x] `index.html` から `/join` 処理を削除
- [x] `index.html` をランディングページに変更
- [x] `_config.yml` で `redirect_from: /join` を設定（既存設定を維持）
- [ ] デプロイして動作確認（デプロイ待ち）
...
1. **✅ `link.profilecode.codes` の実装 - 完了**
   - アプリ側が期待するURLを実装
   - ディープリンクは正常に動作
...
2. **✅ `profilecode.codes/join` のリダイレクト - 完了**
   - 既存のアクセスを新しいURLにリダイレクト
   - 実装完了

3. **✅ `profilecode.codes` のランディングページ化 - 完了**
   - メインドメインを整理
   - 実装完了
